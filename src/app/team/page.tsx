  ...
  const team = [
    {
      ...
      icon: <Code2 className="w-4 h-4 sm:w-5 sm:h-5 text-teal-500" />,
    ...
    },
    {
      ...
      icon: <Layers className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-500" />,
      imageName: "brother.jpg" 
    }
  ];

  return (
    <div className="w-full flex flex-col items-center bg-white min-h-screen pt-24 sm:pt-32 pb-16 sm:pb-24 overflow-hidden">
      
      {/* Background Mesh */}
      <div className="absolute top-0 left-0 w-full h-[300px] sm:h-[500px] bg-gradient-to-b from-slate-50 to-white pointer-events-none -z-10" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12 max-w-5xl mx-auto">
          {team.map((member, index) => (
            <motion.div 
              ...
              className="bg-[#fafafa] rounded-3xl sm:rounded-[2rem] border border-slate-200 p-6 sm:p-8 shadow-sm hover:shadow-xl transition-all duration-300 group"
            >
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 sm:gap-8 mb-6 sm:mb-8">
                
                {/* Profile Image Wrapper */}
                <div className="relative w-24 h-24 sm:w-32 sm:h-32 rounded-full overflow-hidden border-4 border-white shadow-lg shrink-0 bg-slate-200 flex items-center justify-center">
                  <span className="text-[10px] sm:text-xs text-slate-500 text-center px-2">Save {member.imageName}</span>
                ...
                <div className="text-center sm:text-left pt-2">
                  <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 py-1 sm:px-3 sm:py-1 rounded-full bg-white border border-slate-200 shadow-sm mb-2 sm:mb-3">
                    {member.icon}
                    <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-700">{member.role}</span>
                  </div>
                  <h3 className="text-xl sm:text-2xl font-black text-slate-900">{member.name}</h3>
                </div>
              </div>
